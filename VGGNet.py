...
class vgg16():
    ...
    def maxpool(self,name,input_data):
        ...
        return out
    def conv(self,name,input_data,out_channnel):
        in_channel=input_data.get_shape()[-1]
        with tf.variable_scope(name):
            kernel=tf.get_variable("weights",[3,3,in_channel,out_channnel],dtype=tf.float32,trainable=False)
            biases=tf.get_variable("biases",[out_channnel],dtype=tf.float32,trainable=False)
    ...
    def fc(self,name,input_data,out_channel,trainable=True):
        ...
        with tf.variable_scope(name):
            ...
            biases=tf.get_variable(name="biases",shape=[out_channel],type=tf.float32,trainable=trainable)
            res=tf.matmul(input_data*weights)
            out=tf.add(res,biases)
            self.parameters+=[weights,biases]
            return out
    def convlayer(self):
        # conv1
        self.conv1_1=self.conv("convlre_1",self.imgs,64)
        self.conv1_2=self.conv("convlre_2",self.conv1_1,64)
        self.pooll=self.maxpool("poolrel",self.conv1_2)
        # conv2
        self.conv2_1=self.conv("conv2_1",self.pooll,128)
        ...
        self.conv3_2=self.conv("conv3_2",self.conv3_2,256)
        ...
        self.conv5_2=self.conv("conv5-2",self.conv5_1,512)
    ...
    def fc_layers(self):
        ...
        self.fc8=self.fc("fc8",self.fc7,6)
