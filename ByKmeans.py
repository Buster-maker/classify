...
def image2k(imagepath, centors):
    ...
    colors = [[0, 0, 0], [0, 0, 0], [0, 0, 255], [0, 0, 0], [255, 255, 255],
              [255, 255, 0], [0, 255, 255], [255, 0, 255]]
    # (行,列):根据类别设置像素bgr数据
    for i in range(0, row):
        for j in range(0, col):
            ...
            # 当前像素到k个聚类中心的距离
            dists = [distance(img[i][j], centor) for centor in centors]
            for ks in range(k):
                if min(dists) == distance(img[i][j], centors[ks]):
                    if ks==0:
                        img[i][j] = colors[0]
                    elif ks==1:
                        img[i][j] = colors[1]
...
def main(imagepath, k):
    ...
    init_cens=np.array([[0,0,0],[128,255,128],[255,255,255]],'f')
    # 计算k个聚类中心
